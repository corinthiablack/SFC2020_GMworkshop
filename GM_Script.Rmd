...
We could plot all the PCs in 2- and 3-dimentions, but we can also use statistics to see if the total shape is actually different.
...
anova(fit.size, fit.complex) #because the p-value is greater than 0.05 = simple model is best
```

## Allometric analysis
```{r}
## Plot prediction and Regression score
plot(fit.size, type="regression", reg.type = "RegScore", predictor = log(gpa$Csize), bg=classifier$Location, pch=22, cex=1.5)

plot(fit.size, type="regression", reg.type = "PredLine", predictor = log(gpa$Csize), bg=classifier$Location, pch=22, cex=1.5)
```